    ...
    async ({ 
      ...
      try {
        ...
        if (order_id) {
          ...
          return await getOrderDetails(server, session, order_id);
        }
        ...
        if (!data || !data.data || !data.data.orders || !data.data.orders.edges) {
          console.error('Invalid orders data format:', JSON.stringify(data));
          return {
            content: [{ 
              type: 'text', 
...
async function getOrderDetails(server: any, session: Session, id: string): Promise<any> {
  try {
    if (!id || id.trim() === '') {
    ...
    if (!data || !data.data || !data.data.order) {
      console.error('Invalid order data format or order not found:', JSON.stringify(data));
      return {
        content: [{ 
          type: 'text', 
